...
private def rootWithTwoResults := MatchProg.build <| MatchProg.root (.arith .addi) #[]
  #[(⟨10⟩ : Handle OpCode .type), ⟨11⟩]

private def getOperationResultIds (decl : MatchDecl OpCode) : Array Nat :=
  match decl with
  | .operation _ _ _ _ _ _ results => results.map (fun result => result.id)
  | _ => #[]
...
example : rootWithTwoResults.decls[1]?.map getOperationResultIds = some #[1, 2] := by
  native_decide
