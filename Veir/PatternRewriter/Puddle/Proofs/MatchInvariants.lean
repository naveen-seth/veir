...
theorem Assignment.Rooted.runDecl
    ...
    (hmatch : decl.run ctx.raw root assignment = some assignment') :
    Assignment.Rooted assignment' ctx root := by
  cases decl with
  | root opHandle =>
  ...
  | operation opCode operands resultTypes property propertyHandle opHandle results =>
    ...
    have hopPure : operation.Pure ctx.raw := by
      change property.Supported operands.size resultTypes.size ∧ _ at hsupported
...
theorem Assignment.Extends.runDecl
    ...
    (hmatch : decl.run ctx root assignment = some assignment') :
    Assignment.Extends assignment assignment' := by
  cases decl with
  | root opHandle => exact .bindOp hmatch
  | type matcher handle =>
    cases hget : Assignment.getType assignment handle with
    | none => simp [MatchDecl.run, hget] at hmatch
    | some actual =>
...
def MatchDecl.Occurred (decl : MatchDecl OpCode) (ctx : IRContext OpCode)
  ...
  ∃ before after,
    decl.run ctx root before = some after ∧ Assignment.Extends after final
...
theorem MatchProg.runDecls_postconditions
    ...
    (hmatch : MatchProg.runDecls decls ctx root initial = some final) :
  ...
  | nil =>
    ...
    exact ⟨.refl, by simp⟩
  | cons decl decls ih =>
    change (decl.run ctx root initial).bind
      (fun assignment => MatchProg.runDecls decls ctx root assignment) = some final at hmatch
    rw [Option.bind_eq_some_iff] at hmatch
...
theorem Assignment.Rooted.runDecls
    ...
    (h : Assignment.Rooted assignment ctx root)
    (hmatch : MatchProg.runDecls decls ctx.raw root assignment = some assignment') :
    Assignment.Rooted assignment' ctx root := by
  ...
  | nil =>
    ...
    exact h
  | cons decl decls ih =>
    change (decl.run ctx.raw root assignment).bind
      (fun assignment => MatchProg.runDecls decls ctx.raw root assignment) =
        some assignment' at hmatch
    rw [Option.bind_eq_some_iff] at hmatch
    obtain ⟨assignment₁, hdecl, hrest⟩ := hmatch
    apply ih (fun query hmem => hsupported query (by simp [hmem]))
      (h.runDecl ctxDom rootInBounds (hsupported decl (by simp)) hdecl) hrest
...
theorem MatchProg.rooted_of_run
  ...
  Assignment.Rooted.runDecls ctxDom rootInBounds hsupported (.empty prog.numHandles) h

theorem MatchProg.root_mem_of_root?_eq_some
    {prog : MatchProg OpCode α} (hroot : prog.root? = some rootHandle) :
    .root rootHandle ∈ prog.decls := by
  unfold MatchProg.root? at hroot
  have aux : ∀ decls : List (MatchDecl OpCode),
      decls.findSome? (fun | .root root => some root | _ => none) = some rootHandle →
      .root rootHandle ∈ decls := by
    intro decls h
...
theorem MatchDecl.Occurred.root_getOp
    ...
    (h : (MatchDecl.root handle).Occurred ctx rootOp final) :
    Assignment.getOp final handle = some rootOp := by
  obtain ⟨before, after, hmatch, hext⟩ := h
  exact hext.getOp (Assignment.bindOp_get hmatch)
...
theorem MatchProg.supported_root_of_run
    ...
    (hrootMem : .root rootHandle ∈ prog.decls)
    (hrun : prog.run ctx rootOp = some assignment) :
    HasOpInfo.isTerminator (rootOp.getOpType! ctx) = false := by
  obtain ⟨decl, hoperationMem, hsupportsRoot⟩ := hsupported.2 rootHandle hrootMem
  cases decl with
  | root _ => simp [MatchDecl.SupportsRoot] at hsupportsRoot
  ...
  | operation opCode operands resultTypes property propertyHandle opHandle results =>
    ...
    have hoccurs := (MatchProg.runDecls_postconditions hrun).2
    have hrootGet := (hoccurs (.root opHandle) hrootMem).root_getOp
    obtain ⟨before, after, hmatch, hext⟩ :=
      hoccurs (.operation opCode operands resultTypes property propertyHandle opHandle results)
        hoperationMem
    have hbeforeExtendsFinal := (Assignment.Extends.runDecl hmatch).trans hext
    simp [MatchDecl.run] at hmatch
    simp only [Option.bind_eq_some_iff] at hmatch
    obtain ⟨operation, hget, boundAssignment, hbind, resultAssignment, hresults,
      _, hopcode, _, _, _, hproperty, _, _, _, htypes, _, _, hvalues⟩ := hmatch
    ...
    have hopEq : operation = rootOp := by grind
...
theorem MatchProg.supported_operation_of_run
  ...
  obtain ⟨before, after, hmatch, hext⟩ :=
    (MatchProg.runDecls_postconditions hrun).2
      (.operation opCode operands resultTypes property propertyHandle handle results) hoperationMem
  have hbeforeExtendsFinal := (Assignment.Extends.runDecl hmatch).trans hext
  simp [MatchDecl.run] at hmatch
  simp only [Option.bind_eq_some_iff] at hmatch
