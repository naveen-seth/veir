...
private theorem MatchDecl.operation_result_models
    ...
    {rootValues : Array RuntimeValue} {final before after : Assignment OpCode}
    {resultTypes : Array (Handle OpCode .type)} {resultHandles : Array (Handle OpCode .value)}
    {runtimeResult : RuntimeValue}
    (hresultTypes : resultTypes.size = 1)
    (hresultHandles : resultHandles.size = resultTypes.size ∨ resultHandles = #[])
    (hbind : Assignment.bindValues before resultHandles.toList
      (operation.getResults! ctx.raw).toList = some after)
    (hext : Assignment.Extends after final) (hrooted : Assignment.Rooted final ctx root)
  ...
  · have hsize' : resultHandles.size = 1 := hsize.trans hresultTypes
    rcases resultHandles with ⟨resultHandles⟩
    simp only [List.size_toArray] at hsize'
    match resultHandles, hsize' with
    | [resultHandle], _ =>
      have hfinalValue : Assignment.getValue final resultHandle =
          some (.opResult (operation.getResult 0)) := by
        sorry
...
private theorem MatchProg.models_of_run
    ...
    (ctxDom : ctx.Dom) (ctxVerif : ctx.Verified verificationRoot)
  ...
  intro decl hmem
  have hoccurs := (MatchProg.runDecls_postconditions hrun).2 decl hmem
  cases decl with
  | root handle =>
  ...
  | value typeHandle handle =>
    ...
    have hsemanticType := SemanticAssignment.ofConcrete_getType
      (state := state) (root := root) (rootValues := sourceValues) hfinalType
    exact ⟨value.getType! ctx.raw, runtimeValue, hsemanticType, hsemanticValue,
      VariableState.getVar?_conforms hRuntimeValue⟩
  | operation opCode operands resultTypes property propertyHandle handle results =>
    obtain ⟨before, after, hmatchOriginal, hext⟩ := hoccurs
    have hbeforeExtendsFinal :=
      (Assignment.Extends.runDecl hmatchOriginal).trans hext
    ...
    have hdeclSupported :=
      hsupported.1
        (.operation opCode operands resultTypes property propertyHandle handle results) hmem
    change property.Supported operands.size resultTypes.size ∧ _
      at hdeclSupported
    ...
    focus
      ...
      have hresultTypesSize :
          (operation.getResultTypes! ctx.raw).size = resultTypes.size := by
      ...
      have hresultModels := MatchDecl.operation_result_models
        ...
        hbindResults hresultExtendsFinal hrooted (by
          intro hopEq
          have hresultLookup := VariableState.getVar?_getResult_of_setResultValues?
            (varState := state.variables) (varState' := variables)
            (op := operation) (resultValues := #[runtimeResult]) (i := 0) (by grind) hset
...
private theorem Assignment.getType_bindType_eq
    {assignment after : Assignment OpCode} (bound query : Handle OpCode .type) (value : TypeAttr)
    (hbind : Assignment.bindType assignment bound value = some after)
    (heq : query.id = bound.id) :
    Assignment.getType after query = some value := by
...
private theorem Assignment.getType_bindType_ne
    ...
    Assignment.getType after query = Assignment.getType assignment query := by
  unfold Assignment.bindType at hbind
  unfold Assignment.getType Assignment.getBinding
...
private theorem Assignment.getProperty_bindType_eq
    ...
    (query : Handle OpCode (.prop opCode)) (value : TypeAttr)
    (hbind : Assignment.bindType assignment bound value = some after)
    (heq : query.id = bound.id) : Assignment.getProperty after query = none := by
  unfold Assignment.bindType at hbind
...
private theorem Assignment.getProperty_bindType_ne
    ...
    (hbind : Assignment.bindType assignment bound value = some after)
    (hne : query.id ≠ bound.id) :
    Assignment.getProperty after query = Assignment.getProperty assignment query := by
  unfold Assignment.bindType at hbind
  unfold Assignment.getProperty Assignment.getBinding
  rw [Assignment.bind_get_of_ne hbind hne]
...
private theorem Assignment.getType_bindProperty_eq
    ...
    (heq : query.id = bound.id) : Assignment.getType after query = none := by
...
private theorem Assignment.getProperty_bindProperty_eq
    ...
    (hbind : Assignment.bindProperty assignment bound value = some after)
    (heq : query.id = bound.id) :
    Assignment.getProperty after query =
...
private theorem Assignment.MetadataAgrees.bindType
    {concrete after : Assignment OpCode} {semantic : SemanticCreateAssignment}
    (hagrees : (Assignment.MetadataAgrees concrete) semantic)
    (handle : Handle OpCode .type) (value : TypeAttr)
    (hbind : Assignment.bindType concrete handle value = some after) :
    (Assignment.MetadataAgrees after)
      (semantic.bindType handle value) := by
  constructor
  · intro query
    by_cases heq : query.id = handle.id
    · rw [Assignment.getType_bindType_eq _ _ _ hbind heq,
        SemanticCreateAssignment.getType_bindType_eq _ _ _ _ heq]
    · rw [Assignment.getType_bindType_ne _ _ _ hbind heq,
        SemanticCreateAssignment.getType_bindType_ne _ _ _ _ heq]
      exact hagrees.1 query
  · intro opCode query
    by_cases heq : query.id = handle.id
    · rw [Assignment.getProperty_bindType_eq _ _ _ hbind heq,
        SemanticCreateAssignment.getProperty_bindType_eq _ _ _ _ heq]
    · rw [Assignment.getProperty_bindType_ne _ _ _ hbind heq,
...
private theorem Assignment.MetadataAgrees.bindProperty
    ...
    (hagrees : (Assignment.MetadataAgrees concrete) semantic)
    {opCode : OpCode} (handle : Handle OpCode (.prop opCode)) (value : propertiesOf opCode)
    (hbind : Assignment.bindProperty concrete handle value = some after) :
    (Assignment.MetadataAgrees after)
      (semantic.bindProperty handle value) := by
  constructor
  · intro query
    by_cases heq : query.id = handle.id
    · rw [Assignment.getType_bindProperty_eq _ _ _ hbind heq,
        SemanticCreateAssignment.getType_bindProperty_eq _ _ _ _ heq]
    · rw [Assignment.getType_bindProperty_ne _ _ _ hbind heq,
        SemanticCreateAssignment.getType_bindProperty_ne _ _ _ _ heq]
      exact hagrees.1 query
  · intro queryOpCode query
    by_cases heq : query.id = handle.id
    · rw [Assignment.getProperty_bindProperty_eq _ _ _ hbind heq,
        SemanticCreateAssignment.getProperty_bindProperty_eq _ _ _ _ heq]
    · rw [Assignment.getProperty_bindProperty_ne _ _ _ hbind heq,
...
private theorem Assignment.MetadataAgrees.bindOp
  ...
  · intro query
    ...
    · have hc : Assignment.getType after query = none := by
        have hb := Assignment.bind_get hbind
        unfold Assignment.getType Assignment.getBinding
        rw [heq, hb]
        rfl
    ...
    · have hc : Assignment.getType after query = Assignment.getType concrete query := by
        have hp := Assignment.bind_get_of_ne hbind heq
        unfold Assignment.getType Assignment.getBinding
        rw [hp]
    ...
    · have hc : Assignment.getProperty after query = none := by
      ...
      have hs : (semantic.bindOp bound results).getProperty query = none := by
        ...
        · rename_i h
          ...
          simp only [Array.getElem?_append]; simp [hle, heq]
      rw [hc, hs]
    · have hc : Assignment.getProperty after query = Assignment.getProperty concrete query := by
        have hp := Assignment.bind_get_of_ne hbind heq
        unfold Assignment.getProperty Assignment.getBinding
        rw [hp]
...
private theorem Assignment.MetadataAgrees.bindValues
  ...
  | nil =>
    cases values <;> cases runtimes <;> simp_all [Assignment.bindValues,
  ...
  | cons handle handles ih =>
    ...
    | cons value values =>
      ...
      | cons runtime runtimes =>
        ...
        simp only [SemanticCreateAssignment.bindValues]
...
private theorem Assignment.Refines.bindType
  ...
  by_cases heq : query.id = bound.id
  · rw [Assignment.getValue_bindType_eq _ _ _ hbind heq] at hconcrete
    simp at hconcrete
  · rw [Assignment.getValue_bindType_ne _ _ _ hbind heq] at hconcrete
...
private theorem Assignment.Refines.bindOp
    ...
    (hbind : Assignment.bindOp concrete bound operation = some after) :
    (Assignment.Refines after)
      (semantic.bindOp bound results) targetState := by
  intro query value sourceRuntime hconcrete hsemantic
  by_cases heq : query.id = bound.id
  · rw [Assignment.getValue_bindOp_of_eq _ _ _ hbind heq] at hconcrete
    simp at hconcrete
  · rw [Assignment.getValue_bindOp_of_ne _ _ _ hbind heq] at hconcrete
...
private theorem Assignment.Refines.bindValue
  ...
  by_cases heq : query.id = bound.id
  · rw [Assignment.getValue_bindValue_of_eq _ _ _ hbind heq] at hconcrete
  ...
  · rw [Assignment.getValue_bindValue_of_ne _ _ _ hbind heq] at hconcrete
...
private theorem Assignment.Refines.bindProperty
  ...
  intro query value sourceRuntime hconcrete hsemantic
  by_cases heq : query.id = bound.id
  · rw [Assignment.getValue_bindProperty_eq _ _ _ hbind heq] at hconcrete
    simp at hconcrete
  · rw [Assignment.getValue_bindProperty_ne _ _ _ hbind heq] at hconcrete
...
private theorem CreateDecl.interpret
  ...
  | operation opCode operands resultTypes propertySource opHandle results =>
    ...
    have hsemanticProperties :
        SemanticAssignment.getProperty semanticCreated propertySource = some properties := by
      calc
        SemanticAssignment.getProperty semanticCreated propertySource =
            Assignment.getProperty created propertySource := by
          simpa [SemanticCreateAssignment.getProperty] using
            (hmetadata.2 opCode propertySource).symm
        ...
        have hcreatedBaseAfter : (Assignment.Refines created) semanticCreated afterCreation := by
          ...
          exact ⟨targetRuntime,
            (VariableState.getVar?_setResultValues?_of_notMem_getResults!
              (inBounds := hnewOpIn) hnotMem hsetCreated).trans htarget, hrefines⟩
        have hcreatedAfter :
            Assignment.Refines createdWithResults
    ...
    · simp at hrun
  | property => simp [CreateDecl.run] at hrun
-/
    ...
    | some created =>
      ...
      | some resolved =>
        ...
        | some rootHandle =>
          ...
          have hrootMem := MatchProg.root_mem_of_root?_eq_some hroot
          have hrootSupported := MatchProg.supported_root_of_run hsupported hrootMem hrun
          have hrooted := MatchProg.rooted_of_run ctxDom rootIn hsupported.1 hrun
          have hrootOccurs := (MatchProg.runDecls_postconditions hrun).2
            (.root rootHandle) hrootMem
          obtain ⟨rootDecl, hrootOperationMem, hrootDeclSupported⟩ :=
            hsupported.2 rootHandle hrootMem
          have hrootPure := by
            cases rootDecl with
            | root _ => simp [MatchDecl.SupportsRoot] at hrootDeclSupported
            ...
            | operation rootOpCode rootOperands rootReturnTypes rootProperty
              ...
              obtain ⟨rfl, _⟩ := hrootDeclSupported
              exact MatchProg.supported_operation_of_run hsupported hrootOperationMem
